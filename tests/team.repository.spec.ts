...
function makeRepo() {
  return new TeamRepository(
    new InMemoryDataSource<TeamEntity>(),
    new TeamMapper(),
  );
}
...
describe("TeamRepository – save()", () => {
  ...
  it("overwrites an existing team with the same id", async () => {
    ...
    const updated = TeamEntity.build({
      id: id1,
...
// modify()
// ---------------------------------------------------------------------------
describe("TeamRepository – modify()", () => {
  it("updates the team and returns the updated version", async () => {
    ...
    const result = await repo.modify(updated.payload);
  ...
  it("returns null when the team does not exist", async () => {
    ...
    const result = await repo.modify(buildTeam1());
